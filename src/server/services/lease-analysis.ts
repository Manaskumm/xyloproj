...
export async function analyzeContractText(input: { text: string; fileName: string; customCrmRecords: Record<string, string>[] }) {
  const modelName = process.env.GROQ_MODEL || "llama-3.3-70b-versatile";
  const crmRecords = input.customCrmRecords;

  if (!crmRecords || crmRecords.length === 0) {
    throw new Error("CRM records are required. Please upload a CRM CSV file first.");
  }
